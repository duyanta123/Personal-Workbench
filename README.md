...
| 全局搜索 | 跨模块检索并直接定位原始记录 |
 | 数据互通 | V8 流式备份、V1-V7 兼容导入、10 类结构化 CSV 和一次性待办 ICS 日历 |
...
- V8 是桌面端默认的流式 ZIP 格式：manifest、逐表 NDJSON、头像文件和 SHA-256 校验，服务端默认支持单表 500,000 行、总计 2,000,000 行。
- V1-V7 作为兼容输入继续保留原有 JSON 约束：40 MiB 文件上限、单表 50,000 行、总计 200,000 行；旧格式会归一化后进入当前恢复协议。
- V8 的按模块和按年份导出用于查阅，不能直接恢复；移动 Safari 预测输出超过 64 MiB 时必须改用桌面端或分模块导出。
- 所有版本每块最多 500 行、约 900 KiB；头像最多 5 张，单张压缩后不超过 5 MiB。
